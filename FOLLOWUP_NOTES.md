# Follow-up Notes

Use this file to prepare for the follow-up technical discussion.

Before submission, fill this in with:

- Three design decisions you expect to explain.
- One thing you would redesign with more time.
- One generated mistake you caught, or a statement that you didn't use AI tools.
- One unresolved production risk.
- The strongest evidence artifact in your submission.
- Questions you would ask before productionizing the freshness service.
